...
static void ctimer_callback() {
    char set_unset_value[30], anchor_node[26], mobile_node[26];
    ...
    shared_memory_rflx = strtok(shared_memory_down, "\n");
    for(i = 0; i < 16; i++) {
        memset(anchor_node, 0, 26*sizeof(char));
        sscanf(shared_memory_rflx, "%s %s for %s", set_unset_value, anchor_node, mobile_node);
        ...
        simple_udp_sendto(&down_conn, set_unset_value, 30, &anchor_node_address);
        shared_memory_rflx += 55;
    }    
}
